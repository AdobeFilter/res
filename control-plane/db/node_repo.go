...
func (r *pgNodeRepo) Create(ctx context.Context, node *api.NodeInfo) error {
	...
	err := r.pool.QueryRow(ctx,
		`INSERT INTO nodes (account_id, name, node_type, public_key, endpoint, nat_type, internal_ip, status)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		 RETURNING id, created_at`,
		node.AccountID, node.Name, node.NodeType, node.PublicKey,
...
func (r *pgNodeRepo) GetByID(ctx context.Context, id string) (*api.NodeInfo, error) {
	var n api.NodeInfo
	var endpoint, natType, internalIP *string
	err := r.pool.QueryRow(ctx,
		`SELECT id, account_id, name, node_type, public_key, endpoint, nat_type,
		        host(internal_ip), status, last_seen, created_at
		 FROM nodes WHERE id=$1`,
		id,
	).Scan(&n.ID, &n.AccountID, &n.Name, &n.NodeType, &n.PublicKey,
...
func scanNodes(rows pgx.Rows) ([]*api.NodeInfo, error) {
	...
	for rows.Next() {
		var n api.NodeInfo
		var endpoint, natType, internalIP *string
		if err := rows.Scan(&n.ID, &n.AccountID, &n.Name, &n.NodeType, &n.PublicKey,
